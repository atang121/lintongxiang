...
export default function BottomNav() {
  ...
  if (pathname === '/login' || pathname.startsWith('/items/')) {
    return null;
  }
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className="relative flex flex-1 flex-col items-center gap-1 overflow-visible py-1"
              style={{ paddingBottom: 'max(6px, env(safe-area-inset-bottom))' }}
            >
              <span className={`relative flex h-10 w-10 items-center justify-center overflow-visible rounded-2xl transition-all duration-200 ${
                isActive
                ...
                {isMessage && unreadCount > 0 && (
                  <span className="absolute -right-1.5 -top-1.5 z-10 flex h-5 min-w-5 items-center justify-center rounded-full border-2 border-white bg-red-500 px-1 text-[10px] font-bold leading-none text-white shadow-[0_4px_10px_rgba(239,68,68,0.25)]">
