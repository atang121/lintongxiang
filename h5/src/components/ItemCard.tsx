...
import { AGE_LABELS, CATEGORY_LABELS, CONDITION_LABELS, getExchangeModeLabel, getPriceNegotiableLabel, Item, LISTING_TYPE_LABELS } from '@/types';
...
function OfferImageFallback({ item }: { item: Item }) {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-[linear-gradient(135deg,#f7efe4,#eef6f0_52%,#f4f0e8)] px-5 text-center">
      <div className="flex h-16 w-16 items-center justify-center rounded-[22px] bg-white/78 text-3xl shadow-[0_16px_32px_rgba(150,132,102,0.12)]">
        🎁
      </div>
      <p className="line-clamp-2 text-sm font-bold leading-snug text-[#4b5862]">{item.title}</p>
      <p className="text-[11px] font-medium text-[#8c949c]">图片暂不可用</p>
    </div>
  );
}
...
export default function ItemCard({ item, showDistance = true }: ItemCardProps) {
  ...
  const coverImage = imageFailed ? '' : item.images[0];
      ...
      <div className={`group overflow-hidden rounded-[30px] border bg-[rgba(255,253,249,0.96)] shadow-[0_14px_36px_rgba(180,159,136,0.08)] transition-all duration-200 active:scale-[0.985] lg:hover:-translate-y-1 lg:hover:shadow-[0_22px_46px_rgba(180,159,136,0.12)] ${isPending ? 'border-[#e8c97a]' : 'border-[rgba(201,189,171,0.42)]'}`}>
        <div className="relative aspect-[4/4.3] bg-[#f5ece0] sm:aspect-[4/4.6]">
          {coverImage ? (
            ...
            <OfferImageFallback item={item} />
          )}
          <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#5f6b75]/34 to-transparent" />
          {!isWanted && (
            <>
              <div
                className={`absolute left-3 top-3 ${modeBg[item.exchangeMode]} rounded-full px-2.5 py-1 text-[11px] font-bold text-[#54606a] shadow-sm`}
              >
                {modeLabel}
              </div>
          ...
          {!isPending && item.exchangeMode === 'sell' && item.price && (
            <div className="absolute bottom-3 right-3 rounded-2xl bg-white/82 px-3 py-1.5 text-right text-sm font-bold text-[#8f7b58] backdrop-blur-sm">
              <div>{isWanted ? `预算 ¥${item.price}` : `¥${item.price}`}</div>
              {priceNote && <div className="mt-0.5 text-[10px] font-semibold text-[#9c8c6d]">{priceNote}</div>}
            </div>
          )}
        </div>
        ...
        <div className="p-4">
          <div className="mb-2 flex items-start justify-between gap-3">
            ...
            {item.exchangeMode === 'sell' && item.price ? (
              <div className="shrink-0 rounded-full bg-[#fff3df] px-2.5 py-1 text-right text-[12px] font-bold leading-tight text-[#b18e63]">
                <div>{isWanted ? `预算 ¥${item.price}` : `¥${item.price}`}</div>
                {priceNote && <div className="text-[10px] font-semibold text-[#a39376]">{priceNote}</div>}
              </div>
            ) : null}
          </div>
