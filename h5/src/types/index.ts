...
export type PriceNegotiable = 'fixed' | 'negotiable';
...
export interface User {
  ...
  serviceAgreementConfirmedAt?: string;
  serviceAgreementRequired?: boolean;
}
...
export interface Notification {
  id: string;
  type: 'message' | 'exchange' | 'system' | 'feedback' | 'platform' | 'ops' | 'handling';
  title: string;
  content: string;
  createdAt: string;
  read: boolean;
...
export interface Exchange {
  ...
  requesterId: string;
  ownerId: string;
  status: 'pending' | 'waiting' | 'completed' | 'cancelled' | 'failed' | 'expired';
  message?: string;
  createdAt: string;
  updatedAt?: string;
  completedAt?: string;
...
export const PRICE_NEGOTIABLE_LABELS: Record<PriceNegotiable, string> = {
  fixed: '一口价',
  negotiable: '可协商',
};
...
export function getPriceNegotiableLabel(value?: PriceNegotiable, listingType: ListingType = 'offer') {
  if (!value) return '';
  if (value === 'fixed') return listingType === 'wanted' ? '预算固定' : '一口价';
  return listingType === 'wanted' ? '预算可协商' : '可小刀';
}
