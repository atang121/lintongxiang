...
import { Award, Camera, ChevronRight, FileText, MapPin, MessageCircle, Pencil, Settings, ShieldCheck } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const trustLevel = getTrustLevel(currentUser.exchangeCount);
  const stats = [
    { label: '发布物品', value: myItems.length, emoji: '📦' },
    { label: '完成交换', value: currentUser.exchangeCount, emoji: '🤝' },
  ];
  const looksLikeDefaultNickname = /^用户\d+$/.test(currentUser.nickname);
  ...
  return (
    <div className="min-h-screen">
      <div className="page-shell">
        <div className="desktop-float overflow-hidden rounded-[30px] bg-[radial-gradient(circle_at_top_right,_rgba(214,171,98,0.24),_transparent_34%),linear-gradient(145deg,#17362c,#28483a_58%,#385b49)] px-5 pb-5 pt-5 text-white sm:rounded-[34px] sm:px-6 sm:pb-6 sm:pt-6">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-white/62">我的邻里主页</p>
              <h1 className="mt-1 text-xl font-bold">个人中心</h1>
            </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[24px] font-bold leading-tight sm:text-2xl">{currentUser.nickname}</span>
                {currentUser.isLiaison && (
                  <span className="rounded-full bg-amber-400 px-2.5 py-1 text-xs font-bold text-amber-900">
                    🌟 童享大使
                  </span>
                )}
                {looksLikeDefaultNickname && (
                  <Link
                    href="/profile/settings"
                    className="inline-flex items-center gap-1 rounded-full bg-white/14 px-2.5 py-1 text-[11px] font-semibold text-white/86"
                  >
                    <Pencil size={11} /> 完善昵称
                  </Link>
                )}
              ...
              <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/10 px-3 py-1.5 text-xs font-semibold text-white/82">
                <span className="sr-only">邻里等级</span>
          ...
          <div className="mt-5 grid grid-cols-2 divide-x divide-white/15 rounded-[22px] border border-white/12 bg-white/8 backdrop-blur-sm">
        ...
        <section className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
          <div className="paper-surface rounded-[32px] p-5">
            <div>
              ...
              <h2 className="mt-2 text-[22px] font-black leading-tight text-[#4f5a63]">正在流转的物品</h2>
              <p className="mt-1 text-xs text-[#8c949c]">管理自己发布的闲置和需求</p>
            </div>
                  ...
                  {myItems.slice(0, 4).map((item) => (
                    ...
                    >
                      {item.images[0] ? (
                        ...
                        <WantedCover item={item} compact />
                      ) : (
                        <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl bg-[#f7efe4] text-2xl">
                          📦
                        </div>
                      )}
          ...
          <div className="space-y-4">
            <div>
              <p className="mb-2 px-1 text-xs font-semibold text-[#8c949c]">添加到桌面</p>
              <InstallAppPrompt />
            </div>
            ...
            <div className="paper-surface rounded-[32px] p-5">
              <div className="space-y-3">
                {[
                  ...
                  { href: '/terms/service-agreement', icon: FileText, label: '用户服务协议', desc: '平台边界、发布规范、物品承诺、面交安全' },
                  { href: '/privacy?from=profile', icon: FileText, label: '隐私说明', desc: '个人信息收集、使用与注销说明' },
        ...
        <p className="mt-6 text-center text-xs text-[#b6ab97]">童邻市集 · 以利他之心，让闲置流转</p>
      </div>
    </div>
  );
}
