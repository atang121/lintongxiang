...
test('supports publish -> message -> reserve -> complete and creates notifications', async () => {
  ...
  const requesterLoginResponse = await invokeApp(app, {
    method: 'POST',
    url: '/api/auth/login',
    body: {
      nickname: '试玩预约用户',
  ...
  assert.equal(requesterLoginResponse.status, 200);
  const requesterId = requesterLoginResponse.json.data.user.id as string;
  const requesterToken = requesterLoginResponse.json.data.token as string;
  ...
  const messageResponse = await invokeApp(app, {
    ...
    body: {
      item_id: publishResponse.json.data.id,
  ...
  const notificationsResponse = await invokeApp(app, {
    method: 'GET',
    url: `/api/notifications?user_id=${requesterId}`,
    headers: {
      authorization: `Bearer ${requesterToken}`,
    },
  });
