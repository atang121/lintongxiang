...
test('phone-code delivery preview is only exposed outside production', async () => {
  ...
  assert.equal(response.json.data.phone, '13800000001');
  assert.equal(response.json.data.provider, 'preview');
  assert.match(response.json.data.preview_code, /^\d{6}$/);
});

test('phone-code login rejects invalid phone numbers before delivery', async () => {
  await initDatabase({ forceReset: true });
  ...
  assert.equal(response.json.error, '请输入有效的手机号');
});
...
test('ops bootstrap exposes formalized community/config data and authenticated image upload contract', async () => {
  ...
  assert.equal(bootstrapResponse.json.data.communities.length, 5);
  assert.equal(bootstrapResponse.json.data.config.auth_mode, 'phone_code');
