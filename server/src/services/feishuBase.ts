...
async function listTableRecords(tableId: string) {
  ...
  do {
    ...
    allItems = allItems.concat((data.items || []) as FeishuRecord[]);
...
export async function loadOpsConfigFromFeishu(): Promise<Partial<OpsConfig>> {
  ...
  if (keyFieldExists) {
    ...
    return {
      auth_mode: (config.auth_mode as 'qq_email_code') || undefined,
  ...
  return {
    auth_mode:
      (extractFieldValue(getField(firstFields, ['auth_mode', '认证方式'])) as 'qq_email_code') || undefined,
...
export async function createCommunitySubmissionInFeishu(payload: {
  userId: string;
  userNickname: string;
  communityName: string;
  district: string;
}) {
  if (!isFeishuBaseConfigured() || !process.env.FEISHU_COMMUNITY_SUBMISSION_TABLE_ID) {
    return false;
  }

  const fields: Record<string, unknown> = {
    用户ID: payload.userId,
    用户昵称: payload.userNickname,
  ...
  try {
    await createTableRecord(process.env.FEISHU_COMMUNITY_SUBMISSION_TABLE_ID, fields);
    return true;
  } catch (error) {
    console.error('[Feishu] createCommunitySubmission failed:', error);
    return false;
  }
}
