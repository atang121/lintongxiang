...
function getStoragePrefix(category?: string) {
  const normalized = String(category || 'items')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9_-]/g, '');
  const allowed = new Set(['items', 'avatars']);
  return allowed.has(normalized) ? normalized : 'items';
}
...
export async function uploadImage(input: UploadImageInput): Promise<UploadImageResult> {
  ...
  const prefix = getStoragePrefix(input.category);
  const date = new Date();
