...
const communities = [
  { name: '民发・庞公别苑', lat: 32.0078, lng: 112.1264 },
  { name: '梧桐湾', lat: 32.0106, lng: 112.1321 },
  { name: '清华园', lat: 32.0049, lng: 112.1296 },
  { name: '在水一方', lat: 32.0123, lng: 112.1202 },
  { name: '北京公馆', lat: 32.0174, lng: 112.1366 },
  { name: '华凯・丽江泊林', lat: 32.0068, lng: 112.1408 },
  { name: '中房泰鑫花园', lat: 32.0231, lng: 112.1463 },
  { name: '庞公春晓', lat: 32.0019, lng: 112.1188 },
];
...
const featuredPublicListings: FeaturedPublicListing[] = [
  {
    title: '轻便折叠婴儿车',
    description: '车棚、置物篮、脚踏都在，平时主要是小区里短距离遛娃用，前轮顺滑，折叠后放后备箱没压力。孩子已经不用推车了，想转给附近有需要的家庭。',
  ...
  {
    title: '宝宝学步车带防侧翻底盘',
    description: '轮子和音乐都正常，家里宝宝刚会走后就基本闲置了。更适合当过渡用品，附近邻居自提会比较方便。',
...
function createDemoImage(seed: string, title: string, accent: string): string {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="640" height="640" viewBox="0 0 640 640">
      ...
      <text x="64" y="126" fill="#0f172a" font-size="36" font-family="Arial, sans-serif">邻里童享好物</text>
...
function buildFeaturedPublicItems(users: DemoUserSeed[]): DemoItemSeed[] {
  return featuredPublicListings.map((listing, index) => {
    ...
    return {
      ...
      userId: owner.id,
      title: listing.title,
      description: `${listing.description} ${owner.community}附近都可以约时间，自提优先。`,
